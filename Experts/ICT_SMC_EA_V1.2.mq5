...
// Cumulative rejection stats (never reset)
int    cumRejTotal    = 0;
int    cumRejAI       = 0;
int    cumRejMSS      = 0;
int    cumRejBOS      = 0;
...
//===================================================================//
//  AI SIGNAL BRIDGE                                                  //
//  EA writes market state → Python ML model reads + writes result   //
//===================================================================//
...
bool CheckTwinsSequence(bool &isBuy)
{
   ...
   // AI filter check (write request then read signal — before CalculateTradeScore so AI adds bonus)
   if(UseAIFilter)
   {
      ...
      if(!aiOK)
      {
         ...
         if(shouldBlock)
         {
            if(newBar){ rejAI++; rejTotal++; cumRejAI++; cumRejTotal++; rejSeqLastBar=curBar; }
            lastFailedStep=11;
...
void UpdateDisplay()
{
   ...
   // ── ICT SEQUENCE ──────────────────────────────────────────────────────────
   PanelLabel("SeqH",px,y+row*lh+rowTop,"ICT SMC V1.2 SEQUENCE:",PANEL_GOLD);
   ...
   {string _b=PANEL_PREFIX+"SecFRBtn";PanelLabel("SecFRBtn",bx,y+row*lh+rowTop,secFilterHidden?"[+]":"[-]",PANEL_BLUE,8);ObjectSetInteger(0,_b,OBJPROP_SELECTABLE,true);}
   row++;
   if(!secFilterHidden)
   {
...
void OnDeinit(const int reason)
{
   ...
   PrintFilterSummary();   // V1.7: full report to journal
   PanelDeleteAll();
