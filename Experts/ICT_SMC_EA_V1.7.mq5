...
{ if(!PositionSelectByTicket(ticket)) return;
  ...
  req.magic=MAGIC_NUMBER;req.position=ticket;req.comment="ICT SMC V1.7 Partial TP";
...
{ if(IsPositionOpen()) return;
  ...
  bool result=isBuy?trade.Buy(volume,_Symbol,entry,sl,tp,"ICT SMC BUY V1.7"):trade.Sell(volume,_Symbol,entry,sl,tp,"ICT SMC SELL V1.7");
  if(result)
  { TodayTradeCount++;
...
{ if(TimeCurrent()-LastDisplayUpdate<2) return; LastDisplayUpdate=TimeCurrent();
  ...
  // ── ICT SEQUENCE ──
  PanelLabel("SeqH",px,y+row*lh+rowTop,"ICT SMC V1.7 SEQUENCE:",PANEL_GOLD);
...
{ ApplySymbolPreset(); ApplyOptimizationMode(); ApplyTradingStyle();
  ...
  Print("════════════════════════════════════════");
  Print(EA_NAME," — ICT SMART MONEY CONCEPTS V1.7");
