//+------------------------------------------------------------------+
//|                                          ICT SMC EA V1.7          |
//|          ICT SMART MONEY CONCEPTS — FULL MODEL V1.7              |
//|  STRONG/WEAK SWING SL · SWING-BASED SMT DIVERGENCE               |
...
const string EA_NAME      = "ICT SMC EA V1.7";
...
{ MqlDateTime dt; TimeToStruct(TimeCurrent(),dt); if(dt.day==LastTradeDay) return;
  ...
  rejScore=0;rejTrend=0;rejSMT=0;rejNews=0;rejAI=0;rejSeqLastBar=0; }
...
{ if(!PositionSelectByTicket(ticket)) return;
  ...
  req.magic=MAGIC_NUMBER;req.position=ticket;req.comment="ICT SMC V1.6 Partial TP";
...
{ if(IsPositionOpen()) return;
  ...
  bool result=isBuy?trade.Buy(volume,_Symbol,entry,sl,tp,"ICT SMC BUY V1.6"):trade.Sell(volume,_Symbol,entry,sl,tp,"ICT SMC SELL V1.6");
  if(result)
  { TodayTradeCount++;
...
void SaveSectionStates(){GlobalVariableSet(PANEL_PREFIX+"SecBk",secBrokerHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecSeq",secSeqHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecFR",secFilterHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecSw",secSwingsHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecSs",secSessHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecSt",secStatsHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecDD",secDDHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecCfg",secConfigHidden?1:0);}
...
{ if(TimeCurrent()-LastDisplayUpdate<2) return; LastDisplayUpdate=TimeCurrent();
  ...
  // ── ICT SEQUENCE ──
  PanelLabel("SeqH",px,y+row*lh+rowTop,"ICT SMC V1.6 SEQUENCE:",PANEL_GOLD);
...
{ ApplySymbolPreset(); ApplyOptimizationMode(); ApplyTradingStyle();
  ...
  Print("════════════════════════════════════════");
  Print(EA_NAME," — ICT SMART MONEY CONCEPTS V1.6");
