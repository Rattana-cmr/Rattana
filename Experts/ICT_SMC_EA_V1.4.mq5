...
void ApplyTradingStyle()
{
   ...
   switch(TradingStyle)
   {
      ...
      case STYLE_ULTRA_ACTIVE:
         effMinScore=35; effRiskPct=RiskPercent; effCooldown=5; effMinH1Range=10;
         effOTEMin=0.40; effOTEMax=0.95;
...
bool DetectLiquiditySweep(bool &sweepBullish)
{
   ...
   for(int i=1;i<=4;i++)
   { if(m15[i].low<lvlLow-wickPts&&m15[i].close>lvlLow){sweepBullish=true; return true;}
     if(m15[i].high>lvlHigh+wickPts&&m15[i].close<lvlHigh){sweepBullish=false;return true;} }
   return false;
}
...
{ MqlRates d1[4]; bool d1Up=false,d1Dn=false;
  ...
  { IndicatorRelease(h4h); double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID); h4Up=(bid>h4e[0]);h4Dn=(bid<h4e[0]); }
...
{ MqlDateTime dt; TimeToStruct(TimeCurrent(),dt); if(dt.day==LastTradeDay) return;
  ...
  // Reset daily counters only (cumulative untouched)
...
{ if(TimeCurrent()-LastDisplayUpdate<2) return; LastDisplayUpdate=TimeCurrent();
  ...
  // ── ICT SEQUENCE ──
  PanelLabel("SeqH",px,y+row*lh+rowTop,"ICT SMC V1.4 SEQUENCE:",PANEL_GOLD);
