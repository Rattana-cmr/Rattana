...
// V1.6 UPGRADE — AI Signal Bridge
// [39] NEW: UseAIFilter — reads AI predictions from Python bridge file
// [40] NEW: WriteAIRequest() — writes market state to file for Python ML model
// [41] NEW: ReadAISignal() — reads direction + confidence from Python output
// [42] NEW: GetAIScoreBonus() — AI confidence adds up to 20 bonus score points
// [43] NEW: AIRequireSignal — hard gate: block trades when no AI signal available
// [44] NEW: Enhanced CSV export with ML training features (hour, DOW, session, OTE%)
// V1.7 UPGRADE — Strong/Weak Swings + Real SMT Divergence
// [45] NEW: FindStrongSwing() — SL anchors at the swing that actually
//      produced the BOS (the "protected" pullback low/high), instead of
//      whatever M15 pivot happens to be nearest. Falls back to the old
//      nearest-pivot/ATR logic when no confirmed BOS structure exists
//      (e.g. Aggressive/UltraActive styles with structure filters off).
// [46] FIX: CheckSMTDivergence — was comparing a single H1 candle's
//      direction between symbols (fired on noise). Now compares the
//      last two confirmed swing points on each symbol: bullish requires
//      _Symbol making a Lower Low while SMTSymbol makes a Higher Low
//      (and the mirror for bearish), matching the textbook SMT definition.
// V1.8 UPGRADE — Order Blocks + FVG Zones/BPR/CE + Named Killzones
// [47] NEW: DetectOrderBlocks() — last opposite-colour M15 candle before
//      a displacement candle that closes beyond a recent swing high/low.
//      Stored as zones with mitigation tracking; optional hard filter
//      (UseOrderBlockFilter) or just visual confluence + score bonus.
// [48] NEW: DetectFVGZones() — replaces count-only 1M FVG check with
//      real zone storage (top/bottom/time/mitigated). CE (Consequent
//      Encroachment, the 50% level) drives mitigation. BPR (Balanced
...
input group "========== ICT KILLZONES (V1.8) =========="
input bool   UseKillzoneFilter     = false;  // Require price to be inside a named ICT killzone to enter
input bool   KZAsian               = false;  // Asian KZ   ~20:00-00:00 NY time (01:00-05:00 GMT)
input bool   KZLondon              = true;   // London KZ  ~02:00-05:00 NY time (07:00-10:00 GMT)
input bool   KZNewYorkAM           = true;   // NY AM KZ   ~07:00-10:00 NY time (12:00-15:00 GMT)
input bool   KZNewYorkPM           = false;  // NY PM KZ   ~13:30-16:00 NY time (18:30-21:00 GMT)
...
input int    MaxOrderBlocks        = 15;     // Max OB zones tracked at once

input group "========== FVG ZONES / BPR / CE (V1.8) =========="
input bool   ShowFVGZones          = true;   // Draw 1M FVG zones (+ BPR overlap) on chart
input bool   UseFVGZoneFilter      = false;  // Require entry price inside an unmitigated FVG (hard gate)
...
// [V1.8] FVG zones (replaces count-only tracking) + BPR flag
struct FVGZoneT { double top; double bottom; datetime time; bool bullish; bool mitigated; bool bpr; };
FVGZoneT fvgZones[];
int      fvgZoneCount   = 0;
...
int    rejFVGZone=0, cumRejFVGZone=0;  // [V1.8]
...
//===================================================================//
//  [V1.8] NAMED ICT KILLZONES — the 4 windows ICT teaching names     //
//  explicitly (Asian/London/NY AM/NY PM), converted from standard    //
//  EST New York time to GMT. Distinct from the generic session      //
//  filter above: a killzone is a narrower, named timing window used  //
//  as entry confluence, not just "is some session open."             //
//===================================================================//
bool InAsianKillzone()    { double h=GetGMTHour(); return(h>=1.0 &&h<5.0);  }
bool InLondonKillzone()   { double h=GetGMTHour(); return(h>=7.0 &&h<10.0); }
bool InNewYorkAMKillzone(){ double h=GetGMTHour(); return(h>=12.0&&h<15.0); }
bool InNewYorkPMKillzone(){ double h=GetGMTHour(); return(h>=18.5&&h<21.0); }

bool InActiveKillzone()
{
   if(!UseKillzoneFilter) return true;
   if(KZAsian    && InAsianKillzone())     return true;
   if(KZLondon   && InLondonKillzone())    return true;
   if(KZNewYorkAM&& InNewYorkAMKillzone()) return true;
   if(KZNewYorkPM&& InNewYorkPMKillzone()) return true;
   return false;
}

string ActiveKillzoneName()
{
   if(InAsianKillzone())     return "ASIAN";
   if(InLondonKillzone())    return "LONDON";
   if(InNewYorkAMKillzone()) return "NY AM";
   if(InNewYorkPMKillzone()) return "NY PM";
   return "NONE";
}
...
bool DrawKillzoneBox(string name,double gmtStart,double gmtEnd,color c,string label,bool dbg=false)
{
   datetime t1=GetTodayGMTTime(gmtStart), t2=GetTodayGMTTime(gmtEnd);
   if(t2<=t1) { if(dbg) DebugPrint("KZBox "+name+": bad window t1="+TimeToString(t1,TIME_SECONDS)+" t2="+TimeToString(t2,TIME_SECONDS)); return true; } // invalid window config, not a data issue — nothing to draw, no need to retry
   datetime now=TimeCurrent();
   if(now<t1) { ObjectDelete(0,name); ObjectDelete(0,name+"_T"); if(dbg) DebugPrint("KZBox "+name+": not started yet now="+TimeToString(now,TIME_SECONDS)+" t1="+TimeToString(t1,TIME_SECONDS)); return true; } // session hasn't started yet today
   ...
   if(hi<=lo) { if(dbg) DebugPrint("KZBox "+name+": degenerate hi="+DoubleToString(hi,_Digits)+" lo="+DoubleToString(lo,_Digits)); return false; }
   ...
   if(dbg) DebugPrint("KZBox "+name+": drawn OK t1="+TimeToString(t1,TIME_SECONDS)+" t2="+TimeToString(t2,TIME_SECONDS)+" hi="+DoubleToString(hi,_Digits)+" lo="+DoubleToString(lo,_Digits)+" exists="+IntegerToString(ObjectFind(0,name)));
   return true;
}
...
void UpdateKillzoneBoxes()
{
   ...
   static datetime lastDbgLog=0;  // [V1.8] throttle diagnostic logging so DebugMode doesn't spam the journal every tick
   bool dbg=(DebugMode&&TimeCurrent()-lastDbgLog>=10);
   if(dbg) lastDbgLog=TimeCurrent();
   bool allOk=true;
   if(KZAsian)     { if(!DrawKillzoneBox("ICTKZ_ASIA",1.0,5.0,C'40,90,190',"Asian KZ",dbg))     allOk=false; }
   else            { ObjectDelete(0,"ICTKZ_ASIA"); ObjectDelete(0,"ICTKZ_ASIA_T"); }
   if(KZLondon)    { if(!DrawKillzoneBox("ICTKZ_LDN",7.0,10.0,C'40,170,90',"London KZ",dbg))    allOk=false; }
   else            { ObjectDelete(0,"ICTKZ_LDN"); ObjectDelete(0,"ICTKZ_LDN_T"); }
   if(KZNewYorkAM) { if(!DrawKillzoneBox("ICTKZ_NYAM",12.0,15.0,C'210,130,20',"NY AM KZ",dbg))  allOk=false; }
   else            { ObjectDelete(0,"ICTKZ_NYAM"); ObjectDelete(0,"ICTKZ_NYAM_T"); }
   if(KZNewYorkPM) { if(!DrawKillzoneBox("ICTKZ_NYPM",18.5,21.0,C'190,40,180',"NY PM KZ",dbg))  allOk=false; }
   ...
   if(dbg) DebugPrint("KZBox UpdateKillzoneBoxes: allOk="+(allOk?"true":"false")+" barTime="+TimeToString(barTime,TIME_SECONDS)+" lastDraw="+TimeToString(lastDraw,TIME_SECONDS)+" chartPeriod="+EnumToString(_Period));
   ChartRedraw(0);
}
...
//===================================================================//
//  [V1.7] STRONG SWING — the protected pivot that produced the BOS  //
//  Bullish: the pullback Low that formed AFTER the swing High that //
//           price broke (the low Smart Money defended on the way   //
//           up). Bearish: mirror image with Highs.                 //
//  This is the same pivot search DetectBOS() uses internally, run  //
...
bool FindStrongSwing(bool isBuy,double &swPrice)
{
   ...
   if(isBuy  && swLow >0 && swLBar<swHBar){ swPrice=swLow;  return true; }
   if(!isBuy && swHigh>0 && swHBar<swLBar){ swPrice=swHigh; return true; }
   return false;
}

void FindNearestSwing(bool isBuy,double &swPrice)
{ swPrice=0;
  if(FindStrongSwing(isBuy,swPrice))
  { if(ShowSwingLines) DrawSwingLine(swPrice,isBuy,"M15-Strong"); return; }
...
//  a displacement candle that closes beyond a recent swing high/low. //
//  Stored as zones (top/bottom/time) with mitigation tracking, so    //
//  price returning into an unmitigated zone can be used as visual    //
//  confluence, a score bonus, or an optional hard entry filter.      //
//===================================================================//
void DrawOrderBlock(int idx)
{
   string name="ICTOB_"+IntegerToString(idx);
...
void AddOrderBlock(double top,double bottom,datetime t,bool bullish)
{
   if(MaxOrderBlocks<=0) return;
   if(obCount>0 && obZones[0].time==t && obZones[0].bullish==bullish) return;
...
// Scans once per closed M15 bar: was the last close a displacement candle
// that broke a recent swing? If so, the last opposite-colour candle before
// it is the order block.
void DetectOrderBlocks()
{
   if(!ShowOrderBlocks && !UseOrderBlockFilter) return;
   int need=OBLookbackBars+10;
   MqlRates m[]; ArraySetAsSeries(m,true);
   if(CopyRates(_Symbol,PERIOD_M15,0,need,m)<need) return;
   ...
   if(body<atr*OBDisplacementATRMulti) return;
   bool bull=(m[1].close>m[1].open);

   double swHigh=0,swLow=0; int conf=3;
   for(int i=2+conf;i<need-conf;i++)
   {
      bool isH=true,isL=true;
      for(int j=i-conf;j<=i+conf;j++)
      { if(j==i||j<2||j>=need) continue;
        if(m[j].high>=m[i].high) isH=false;
        if(m[j].low <=m[i].low)  isL=false; }
      if(isH&&(swHigh==0||m[i].high>swHigh)) swHigh=m[i].high;
      if(isL&&(swLow==0 ||m[i].low <swLow))  swLow=m[i].low;
   }

   if(bull && swHigh>0 && m[1].close>swHigh)
   { for(int k=2;k<=9 && k<need;k++)
     { if(m[k].close<m[k].open){ AddOrderBlock(m[k].high,m[k].low,m[k].time,true); break; } } }
   else if(!bull && swLow>0 && m[1].close<swLow)
   { for(int k=2;k<=9 && k<need;k++)
     { if(m[k].close>m[k].open){ AddOrderBlock(m[k].high,m[k].low,m[k].time,false); break; } } }
}
...
// Unconditional presence check (ignores UseOrderBlockFilter) — used for the score bonus.
bool IsPriceNearOrderBlock(bool isBuy)
{
   double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);
   for(int i=0;i<obCount;i++)
   { if(obZones[i].mitigated || obZones[i].bullish!=isBuy) continue;
     if(bid<=obZones[i].top && bid>=obZones[i].bottom) return true; }
   return false;
}
// Hard-gate version — bypasses when the filter is off.
bool IsPriceInOrderBlock(bool isBuy) { if(!UseOrderBlockFilter) return true; return IsPriceNearOrderBlock(isBuy); }
...
//===================================================================//
//  [V1.8] FVG ZONES + BPR/CE — replaces the count-only 1M FVG check  //
//  with real zone storage (top/bottom/time/mitigated). CE            //
//  (Consequent Encroachment, the 50% level) drives mitigation: a     //
//  zone is considered filled once price closes through its midpoint //
//  rather than the full range. BPR (Balanced Price Range) is flagged //
//  when a new bullish/bearish FVG overlaps an existing unmitigated   //
//  zone of the opposite direction.                                   //
//===================================================================//
double FVGZoneCE(int idx) { return (fvgZones[idx].top+fvgZones[idx].bottom)/2.0; }

void DrawFVGZone(int idx)
{
   string name="ICTFVG_"+IntegerToString(idx);
   datetime t1=fvgZones[idx].time+(datetime)(PeriodSeconds(PERIOD_M1)*30);
   if(ObjectFind(0,name)>=0) ObjectDelete(0,name);
   ObjectCreate(0,name,OBJ_RECTANGLE,0,fvgZones[idx].time,fvgZones[idx].top,t1,fvgZones[idx].bottom);
   color c;
   if(fvgZones[idx].mitigated) c=fvgZones[idx].bullish?C'25,50,35':C'50,25,25';
   else if(fvgZones[idx].bpr&&ShowBPRZones) c=clrGold;
   else                        c=fvgZones[idx].bullish?clrDodgerBlue:clrMagenta;
   ObjectSetInteger(0,name,OBJPROP_COLOR,c);
   ObjectSetInteger(0,name,OBJPROP_FILL,!fvgZones[idx].mitigated);
   ObjectSetInteger(0,name,OBJPROP_BACK,true);
   ObjectSetInteger(0,name,OBJPROP_SELECTABLE,false);
...
void AddFVGZone(double top,double bottom,datetime t,bool bullish)
{
   if(MaxFVGZones<=0) return;
   if(fvgZoneCount>0 && fvgZones[0].time==t && fvgZones[0].bullish==bullish) return;

   bool bpr=false;
   for(int i=0;i<fvgZoneCount;i++)
   { if(fvgZones[i].mitigated || fvgZones[i].bullish==bullish) continue;
     if(top>=fvgZones[i].bottom && bottom<=fvgZones[i].top) { bpr=true; fvgZones[i].bpr=true; break; } }

   int n=(int)MathMin(fvgZoneCount+1,MaxFVGZones);
   for(int i=n-1;i>0;i--) fvgZones[i]=fvgZones[i-1];
   fvgZones[0].top=top; fvgZones[0].bottom=bottom; fvgZones[0].time=t;
   fvgZones[0].bullish=bullish; fvgZones[0].mitigated=false; fvgZones[0].bpr=bpr;
   fvgZoneCount=n;
   if(ShowFVGZones) RedrawAllFVGZones();
}

// Scans once per closed M1 bar for a 3-candle FVG (matches the original
// CountFVGsOn1Min ascending-order convention, run here on a descending series).
void DetectFVGZones()
{
   if(!ShowFVGZones && !UseFVGZoneFilter) return;
   int need=FVGZoneLookbackBars+10;
   MqlRates m[]; ArraySetAsSeries(m,true);
   if(CopyRates(_Symbol,PERIOD_M1,0,need,m)<4) return;
   if(lastFVGScanBar==m[1].time) return;
   lastFVGScanBar=m[1].time;

   if(m[1].low>m[3].high)  AddFVGZone(m[1].low, m[3].high,m[2].time,true);
   if(m[1].high<m[3].low)  AddFVGZone(m[3].low, m[1].high,m[2].time,false);
}
...
void UpdateFVGMitigation()
{
   ...
   for(int i=0;i<fvgZoneCount;i++)
   {
      ...
      double ce=FVGZoneCE(i);
      bool filled=fvgZones[i].bullish?(bid<=ce):(ask>=ce);
      if(filled) { fvgZones[i].mitigated=true; if(ShowFVGZones) DrawFVGZone(i); }
   }
}
...
{
   int c=0;
   for(int i=0;i<fvgZoneCount;i++) if(fvgZones[i].bpr && !fvgZones[i].mitigated) c++;
   return c;
}

// Unconditional presence check (ignores UseFVGZoneFilter) — used for the score bonus.
bool IsPriceNearFVGZone(bool isBuy)
{
   double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);
   for(int i=0;i<fvgZoneCount;i++)
   { if(fvgZones[i].mitigated || fvgZones[i].bullish!=isBuy) continue;
     if(bid<=fvgZones[i].top && bid>=fvgZones[i].bottom) return true; }
   return false;
}
// Hard-gate version — bypasses when the filter is off.
bool IsPriceInFVGZone(bool isBuy) { if(!UseFVGZoneFilter) return true; return IsPriceNearFVGZone(isBuy); }

//===================================================================//
//  [V1.7] SMT DIVERGENCE — last two confirmed swings, not one candle //
//  Bullish: _Symbol prints a Lower Low while SMTSymbol prints a      //
//           Higher Low over the same window (correlated asset fails  //
//           to confirm new weakness → divergence).                   //
//  Bearish: mirror image — Higher High vs Lower High.                //
//===================================================================//
bool GetLastTwoSwingExtremes(string sym,bool wantLows,double &recent,double &prior)
{
   int lookback=40, conf=3, need=lookback+conf*2+5;
   MqlRates r[]; ArraySetAsSeries(r,true);
   if(CopyRates(sym,PERIOD_M15,0,need,r)<need) return false;
   double vals[2]={0,0}; int found=0;
   for(int i=conf;i<lookback-conf && found<2;i++)
   {
      bool ok=true;
      for(int j=i-conf;j<=i+conf;j++)
      {
         if(j==i||j<0||j>=need) continue;
         if(wantLows  && r[j].low <=r[i].low ){ ok=false; break; }
         if(!wantLows && r[j].high>=r[i].high){ ok=false; break; }
      }
      if(ok) vals[found++]=wantLows?r[i].low:r[i].high;
   }
   if(found<2) return false;
   recent=vals[0]; prior=vals[1];
   return true;
}
...
bool CheckSMTDivergence(bool isBuy)
{
   ...
   if(isBuy)
   {
      if(!GetLastTwoSwingExtremes(_Symbol,true,mainRecent,mainPrior))   return true;
      if(!GetLastTwoSwingExtremes(SMTSymbol,true,corrRecent,corrPrior)) return true;
      return(mainRecent<mainPrior)&&(corrRecent>corrPrior); // LL vs HL
   }
   if(!GetLastTwoSwingExtremes(_Symbol,false,mainRecent,mainPrior))    return true;
   if(!GetLastTwoSwingExtremes(SMTSymbol,false,corrRecent,corrPrior))  return true;
...
{ int s=0;
  ...
  s+=15; if(IsTrendAligned(isBuy)) s+=10;
  s+=GetAIScoreBonus(isBuy);  // [V1.6] AI confidence bonus: 0-20 pts
  if(IsPriceNearOrderBlock(isBuy)) s+=5;  // [V1.8]
...
void RejAI()      { rejAI++;    rejTotal++;cumRejAI++;     cumRejTotal++; }  // [V1.6]
void RejKZ()      { rejKZ++;     rejTotal++;cumRejKZ++;     cumRejTotal++; }  // [V1.8]
void RejOB()      { rejOB++;     rejTotal++;cumRejOB++;     cumRejTotal++; }  // [V1.8]
void RejFVGZone() { rejFVGZone++;rejTotal++;cumRejFVGZone++;cumRejTotal++; }  // [V1.8]
...
{
   DetectOrderBlocks();  // [V1.8] runs once per closed M15 bar regardless of context state
...
bool ReadAISignal(bool isBuy)
{
   ...
   { datetime sigTime=StringToTime(parts[2]);
     if(sigTime>0&&TimeCurrent()-sigTime>(datetime)(AISignalMaxAgeMin*60)) return false; // stale
...
bool CheckTwinsSequence(bool &isBuy)
{
   ...
   // [V1.8] Order Block / FVG zone confluence (optional hard filters)
   if(!IsPriceInOrderBlock(isBuy))
   { if(newBar){RejOB();rejSeqLastBar=curBar;} lastFailedStep=9;lastFailedStepDesc="Order Block";return false; }
   if(!IsPriceInFVGZone(isBuy))
   { if(newBar){RejFVGZone();rejSeqLastBar=curBar;} lastFailedStep=9;lastFailedStepDesc="FVG Zone";return false; }
   ...
   // [V1.6] Write AI request then read signal (must be BEFORE CalculateTradeScore — AI adds bonus)
   if(UseAIFilter)
   { WriteAIRequest(isBuy);
     ...
     { bool shouldBlock=aiSignalValid||AIRequireSignal;
       if(shouldBlock)
       { if(newBar){RejAI();rejSeqLastBar=curBar;}
         lastFailedStep=11;
...
{ MqlDateTime dt; TimeToStruct(TimeCurrent(),dt); if(dt.day==LastTradeDay) return;
  ...
  rejScore=0;rejTrend=0;rejSMT=0;rejNews=0;rejAI=0;
  rejKZ=0;rejOB=0;rejFVGZone=0;rejSeqLastBar=0; }
...
//  Fields cover all ICT filters, session, trend, AI — ready for ML training.
//===================================================================//
...
{ if(!EnableCSVLog) return;
  ...
  FileWrite(fh,
    ...
    IntegerToString(GetAIScoreBonus(isBuy)),IntegerToString(score),
    allowed?"YES":"NO",rejReason);
  FileClose(fh); }

//===================================================================//
//  [ML] MFE / MAE TRACKER  — called every tick while position is open
//  Stores max favorable and max adverse excursion (pips) per position
//  in GlobalVariables, keyed by position ID.
//===================================================================//
void UpdateMFEMAE()
{ for(int i=PositionsTotal()-1;i>=0;i--)
...
//  Written at trade close. Entry context saved in GlobalVars at open time.
//  Includes MFE, MAE, P/L in R — ready for ML training.
//===================================================================//
...
void PrintEntryReport(bool isBuy)
{
   ...
   string tStr = !effUseDailyTrend?"DISABLED":(hDir?(IsTrendAligned(isBuy)?"PASS":"FAIL"):"N/A (direction unknown)");
   bool   sOK  = !UseTradeScore||effMinScore<=0||lastTradeScore>=effMinScore;

   Print("┌─────────────── ENTRY REPORT ───────────────");
   Print("│ Time       : ",TimeToString(TimeCurrent(),TIME_DATE|TIME_MINUTES));
   Print("│ Symbol     : ",_Symbol,"  Style: ",EnumToString(TradingStyle));
   Print("├────────────────────────────────────────────");
   Print("│ HTF Level  : ",!HTFLevelRequired?"DISABLED":(htfLevelReached?"PASS":"FAIL"));
   Print("│ MSS (H1)   : ",!effUseMSSFilter?"DISABLED":(mssConfirmed?("PASS ["+(mssIsBullish?"BULL":"BEAR")+"]"):"FAIL"));
   Print("│ BOS (M15)  : ",!effUseBOSFilter?"DISABLED":(bosConfirmed?("PASS ["+(bosIsBullish?"BULL":"BEAR")+"]"):"FAIL"));
   Print("│ Liq Sweep  : ",!effRequireLiqSweep?"DISABLED":(liquiditySweepDone?"PASS":"FAIL"));
   ...
   Print("│ Killzone   : ",!UseKillzoneFilter?"DISABLED":(InActiveKillzone()?("PASS ["+ActiveKillzoneName()+"]"):"FAIL"),"  (now: ",ActiveKillzoneName(),")");
   Print("│ Order Block: ",!UseOrderBlockFilter?"DISABLED":(IsPriceInOrderBlock(isBuy)?"PASS":"FAIL")," (",obCount,"/",MaxOrderBlocks," zones)");
   Print("│ FVG Zone   : ",!UseFVGZoneFilter?"DISABLED":(IsPriceInFVGZone(isBuy)?"PASS":"FAIL")," (",fvgZoneCount,"/",MaxFVGZones," zones, BPR:",CountActiveBPR(),")");
   Print("├────────────────────────────────────────────");
...
void PrintFilterSummary()
{
   ...
   if(cumRejAI>maxRej)     {maxRej=cumRejAI;      topBlocker="AI Filter";}
   if(cumRejKZ>maxRej)     {maxRej=cumRejKZ;      topBlocker="Killzone";}
   if(cumRejOB>maxRej)     {maxRej=cumRejOB;      topBlocker="Order Block";}
   if(cumRejFVGZone>maxRej){maxRej=cumRejFVGZone; topBlocker="FVG Zone";}
   ...
   Print("║  AI Blocks    : ",cumRejAI," (UseAIFilter=",UseAIFilter,")");
   Print("║  Killzone     : ",cumRejKZ," (UseKillzoneFilter=",UseKillzoneFilter,")");
   Print("║  Order Block  : ",cumRejOB," (UseOrderBlockFilter=",UseOrderBlockFilter,")");
   Print("║  FVG Zone     : ",cumRejFVGZone," (UseFVGZoneFilter=",UseFVGZoneFilter,")");
   Print("╠══════════════════════════════════════════╣");
   Print("╠══════════════════════════════════════════╣");
...
{ if(IsPositionOpen()) return;
  ...
  { TodayTradeCount++;
    ...
    if(od>0&&HistoryDealSelect(od)){ulong posID=(ulong)HistoryDealGetInteger(od,DEAL_POSITION_ID);
      ...
      // [ML] Save trade context for ML data collection (read back at close time)
      ...
      GlobalVariableSet("ICTSMC_SLP_"+_p,slPts);
...
void SaveSectionStates(){GlobalVariableSet(PANEL_PREFIX+"SecBk",secBrokerHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecSeq",secSeqHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecFR",secFilterHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecSw",secSwingsHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecSs",secSessHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecSt",secStatsHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecDD",secDDHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecCfg",secConfigHidden?1:0);GlobalVariableSet(PANEL_PREFIX+"SecZn",secZonesHidden?1:0);}
void LoadSectionStates(){if(GlobalVariableCheck(PANEL_PREFIX+"SecBk"))secBrokerHidden=GlobalVariableGet(PANEL_PREFIX+"SecBk")>0.5;if(GlobalVariableCheck(PANEL_PREFIX+"SecSeq"))secSeqHidden=GlobalVariableGet(PANEL_PREFIX+"SecSeq")>0.5;if(GlobalVariableCheck(PANEL_PREFIX+"SecFR"))secFilterHidden=GlobalVariableGet(PANEL_PREFIX+"SecFR")>0.5;if(GlobalVariableCheck(PANEL_PREFIX+"SecSw"))secSwingsHidden=GlobalVariableGet(PANEL_PREFIX+"SecSw")>0.5;if(GlobalVariableCheck(PANEL_PREFIX+"SecSs"))secSessHidden=GlobalVariableGet(PANEL_PREFIX+"SecSs")>0.5;if(GlobalVariableCheck(PANEL_PREFIX+"SecSt"))secStatsHidden=GlobalVariableGet(PANEL_PREFIX+"SecSt")>0.5;if(GlobalVariableCheck(PANEL_PREFIX+"SecDD"))secDDHidden=GlobalVariableGet(PANEL_PREFIX+"SecDD")>0.5;if(GlobalVariableCheck(PANEL_PREFIX+"SecCfg"))secConfigHidden=GlobalVariableGet(PANEL_PREFIX+"SecCfg")>0.5;if(GlobalVariableCheck(PANEL_PREFIX+"SecZn"))secZonesHidden=GlobalVariableGet(PANEL_PREFIX+"SecZn")>0.5;}
...
{ if(TimeCurrent()-LastDisplayUpdate<2) return; LastDisplayUpdate=TimeCurrent();
  ...
  PanelRect("BG",x,yb,w,panelHidden?hdrH:990,PANEL_BG,PANEL_BORDER);
  ...
  PanelLabelC("Author",yb+28,"Created by: RATTANA CHHORM",clrWhite,9);
  PanelLabel("ToggleBtn",x+w-50,yb+4,"[hide]",PANEL_BLUE,8); ObjectSetInteger(0,PANEL_PREFIX+"ToggleBtn",OBJPROP_SELECTABLE,true);
  PanelLabel("Header",px,yb+4,"[drag]",C'50,50,70',7); ObjectSetInteger(0,PANEL_PREFIX+"Header",OBJPROP_SELECTABLE,true);
  int bx=x+w-28;
  row=0;
  ...
  // ── FILTER REJECTIONS ──
  PanelLabel("FRH",px,y+row*lh+rowTop,"FILTER REJECTIONS (today):",PANEL_GOLD);
  {string _b=PANEL_PREFIX+"SecFRBtn";PanelLabel("SecFRBtn",bx,y+row*lh+rowTop,secFilterHidden?"[+]":"[-]",PANEL_BLUE,8);ObjectSetInteger(0,_b,OBJPROP_SELECTABLE,true);} row++;
  if(!secFilterHidden){
    ...
    PanelLabel("FR4v",px,y+row*lh+rowTop,"AI :"+IntegerToString(rejAI)+" (today) cum:"+IntegerToString(cumRejAI),C'150,100,200'); row++;
    PanelLabel("FR5v",px,y+row*lh+rowTop,"KZ :"+IntegerToString(rejKZ)+" OB:"+IntegerToString(rejOB)+" FVGz:"+IntegerToString(rejFVGZone),PANEL_BLUE); row++;
  ...
  // ── ICT ZONES (V1.8) ──
  PanelLabel("ZnH",px,y+row*lh+rowTop,"ICT ZONES (V1.8):",PANEL_GOLD);
  {string _b=PANEL_PREFIX+"SecZnBtn";PanelLabel("SecZnBtn",bx,y+row*lh+rowTop,secZonesHidden?"[+]":"[-]",PANEL_BLUE,8);ObjectSetInteger(0,_b,OBJPROP_SELECTABLE,true);} row++;
  if(!secZonesHidden){
    string kzv=!UseKillzoneFilter?"OFF: "+ActiveKillzoneName():ActiveKillzoneName(); color kzc=InActiveKillzone()?PANEL_GREEN:PANEL_TXT;
    PanelLabel("ZKl",px,y+row*lh+rowTop,"Killzone   :",PANEL_TXT); PanelLabel("ZKv",vx,y+row*lh+rowTop,kzv,kzc); row++;
    PanelLabel("ZOl",px,y+row*lh+rowTop,"Order Blks :",PANEL_TXT); PanelLabel("ZOv",vx,y+row*lh+rowTop,IntegerToString(obCount)+"/"+IntegerToString(MaxOrderBlocks)+(UseOrderBlockFilter?" [FILTER]":""),obCount>0?PANEL_GREEN:PANEL_TXT); row++;
    PanelLabel("ZFl",px,y+row*lh+rowTop,"FVG Zones  :",PANEL_TXT); PanelLabel("ZFv",vx,y+row*lh+rowTop,IntegerToString(fvgZoneCount)+"/"+IntegerToString(MaxFVGZones)+" BPR:"+IntegerToString(CountActiveBPR())+(UseFVGZoneFilter?" [FILTER]":""),fvgZoneCount>0?PANEL_GREEN:PANEL_TXT); row++;
  }
  // ── SESSIONS ──
...
{ if(id==CHARTEVENT_OBJECT_CLICK&&sparam==PANEL_PREFIX+"ToggleBtn")
  ...
  if(id==CHARTEVENT_OBJECT_CLICK&&sparam==PANEL_PREFIX+"SecBkBtn") {secBrokerHidden=!secBrokerHidden;SaveSectionStates();panelDirty=true;LastDisplayUpdate=0;UpdateDisplay();return;}
  if(id==CHARTEVENT_OBJECT_CLICK&&sparam==PANEL_PREFIX+"SecSeqBtn"){secSeqHidden=!secSeqHidden;SaveSectionStates();panelDirty=true;LastDisplayUpdate=0;UpdateDisplay();return;}
  if(id==CHARTEVENT_OBJECT_CLICK&&sparam==PANEL_PREFIX+"SecFRBtn") {secFilterHidden=!secFilterHidden;SaveSectionStates();panelDirty=true;LastDisplayUpdate=0;UpdateDisplay();return;}
  if(id==CHARTEVENT_OBJECT_CLICK&&sparam==PANEL_PREFIX+"SecSwBtn") {secSwingsHidden=!secSwingsHidden;SaveSectionStates();panelDirty=true;LastDisplayUpdate=0;UpdateDisplay();return;}
  if(id==CHARTEVENT_OBJECT_CLICK&&sparam==PANEL_PREFIX+"SecSsBtn") {secSessHidden=!secSessHidden;SaveSectionStates();panelDirty=true;LastDisplayUpdate=0;UpdateDisplay();return;}
  if(id==CHARTEVENT_OBJECT_CLICK&&sparam==PANEL_PREFIX+"SecStBtn") {secStatsHidden=!secStatsHidden;SaveSectionStates();panelDirty=true;LastDisplayUpdate=0;UpdateDisplay();return;}
  if(id==CHARTEVENT_OBJECT_CLICK&&sparam==PANEL_PREFIX+"SecDDBtn") {secDDHidden=!secDDHidden;SaveSectionStates();panelDirty=true;LastDisplayUpdate=0;UpdateDisplay();return;}
  if(id==CHARTEVENT_OBJECT_CLICK&&sparam==PANEL_PREFIX+"SecCfgBtn"){secConfigHidden=!secConfigHidden;SaveSectionStates();panelDirty=true;LastDisplayUpdate=0;UpdateDisplay();return;}
  if(id==CHARTEVENT_OBJECT_CLICK&&sparam==PANEL_PREFIX+"SecZnBtn") {secZonesHidden=!secZonesHidden;SaveSectionStates();panelDirty=true;LastDisplayUpdate=0;UpdateDisplay();return;}
...
{ ApplySymbolPreset(); ApplyOptimizationMode(); ApplyTradingStyle();
  ...
  if(UseAIFilter)
  { Print("AI Filter: ON | SignalFile: ",AISignalFile);
    Print("AI MinConf: ",DoubleToString(AIMinConfidence,2)," | MaxAge: ",AISignalMaxAgeMin,"m | Required: ",AIRequireSignal); }
  Print("Killzone Filter: ",UseKillzoneFilter," | Asian:",KZAsian," London:",KZLondon," NY AM:",KZNewYorkAM," NY PM:",KZNewYorkPM); // [V1.8]
  Print("Order Blocks: Show=",ShowOrderBlocks," Filter=",UseOrderBlockFilter," Max=",MaxOrderBlocks); // [V1.8]
  Print("FVG Zones: Show=",ShowFVGZones," Filter=",UseFVGZoneFilter," Max=",MaxFVGZones," BPR=",ShowBPRZones); // [V1.8]
  Print("════════════════════════════════════════");
...
{ UpdateDisplay(); CheckFridayClose(); CheckPartialTP(); ApplyBreakeven(); ApplyTrailingStop();
  ...
  { lastSignalBar=repBar;
    WriteSignalLog(isBuy,false,lastFailedStepDesc,lastTradeScore);  // [ML] log rejected signal
