...
input double   InpMaxRiskOvershoot = 3.0;  // skip a trade if the broker's min-lot floor would force real $ risk above this multiple of the intended risk% (0 = no cap)
...
int OnInit()
{
   ...
   Print("========================================");
   Print("LIQUIDITY SWEEP EA v7.12 - HIGH FREQUENCY");
...
bool CheckRiskAndEntry(int idx)
{
   ...
   else
   {
      ...
      riskMoney = (riskPercent / 100.0) * AccountInfoDouble(ACCOUNT_BALANCE);
   ...
   // broker min-lot floor can force real risk far above the intended target (esp. gold on small accounts) — skip rather than take an oversized trade
   if(riskMoney > 0 && InpMaxRiskOvershoot > 0 && symbols[idx].riskAmount > riskMoney * InpMaxRiskOvershoot)
      return false;
...
void CreateDashboard()
{
   ...
   if(ObjectCreate(0, "DB_Version", OBJ_LABEL, 0, 0, 0))
   {
      ...
      ObjectSetString(0,  "DB_Version", OBJPROP_TEXT,      "V7.12 BOS: " + BOSModeText(InpBOSMode));
   }
