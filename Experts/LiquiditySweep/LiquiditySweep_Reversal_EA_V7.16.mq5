...
input bool     InpIncludeGold    = false;  // was true - CONFLUENCE EA now trades XAUUSD on this account; running both on gold simultaneously doubles real exposure with neither EA aware of the other
...
input double   InpMaxRiskPercentCap = 2.0;  // hard ceiling on real $ risk per trade, as % of current balance — independent of InpRiskPercent (0 = no cap); catches broker min-lot floor distortion without throttling normal FX trades
...
input double   InpBreakEvenR      = 1.0;  // was 0.5 - on small-account min-lot trades (no partial-TP split), 0.5R of a tight stop is within normal noise and flattens winners before they develop
...
input ENUM_TREND_FILTER InpTrendFilter = SMA_50;
input double   InpMinATR          = 1.0;

input group "=== ULTRA MODE OPTIONS ==="
input ENUM_BOS_MODE InpBOSMode   = BOS_RELAXED;  // require a real swing-pivot + directional candle close (no added bar delay) instead of firing on any wick

input group "=== Order Block Confirmation (two-step entry) ==="
input bool     InpRequireOBConfirm     = true;   // after sweep+BOS confirms, don't enter on the same bar — wait for price to retrace and close back through the recent counter-swing on InpConfTF first (the "order block" retest)
input int      InpOBLookbackBars       = 8;      // InpConfTF bars scanned back to locate the counter-swing level that price must close back through
input int      InpOBConfirmTimeoutBars = 10;     // abandon the pending setup if no confirming close happens within this many InpConfTF bars

input group "=== Candle Quality Filter ==="
input bool     InpUseCandleFilter   = true;
input double   InpMinWickRatio      = 0.35;  // sweep wick must be at least this fraction of the candle's range — filters candles that trended through without real rejection
input double   InpMinBodyRatio      = 0.10;  // candle body must be at least this fraction of range — filters dojis/indecision candles
...
input bool     InpUseSessionFilter   = false;   // master switch — false = trade 24h
...
struct SymbolData
{
   ...
   bool            pendingOBConfirm; // sweep+BOS confirmed, waiting for lower-TF order-block retest before executing
   double          obLevel;          // counter-swing level price must close back through to confirm
   datetime        obLastBarTime;    // last InpConfTF bar already evaluated, avoids re-checking mid-bar
...
void InitSymbolData(int idx)
{
   ...
   symbols[idx].pendingOBConfirm = false;
   symbols[idx].obLevel = 0;
   symbols[idx].obLastBarTime = 0;
...
// Pin-bar/rejection shape: long wick beyond the swept level + a real (non-doji) body
bool CheckCandleQuality(double o, double h, double l, double c, bool isShort)
{
   if(!InpUseCandleFilter) return true;
   double range = h - l;
   if(range <= 0) return false;
   double body = MathAbs(c - o);
   double sweepWick = isShort ? (h - MathMax(o, c)) : (MathMin(o, c) - l);
   if(sweepWick / range < InpMinWickRatio) return false;
...
double GetUSDExposure(string symbol, bool isLong)
{
   ...
   if(StringFind(symbol, "JPY") >= 0) return isLong ? 1 : -1;     // JPY-cross (EURJPY/GBPJPY/AUDJPY/CADJPY): long = short JPY, same bucket as USDJPY long
   if(StringFind(symbol, "XAUUSD") >= 0) return isLong ? -1 : 1; // gold: long = USD-bearish sentiment, same bucket as EURUSD long
   return 0;
}
...
void OnTick()
{
   ...
   for(int i = 0; i < ArraySize(symbols); i++)
      if(symbols[i].pendingOBConfirm)
         CheckOBConfirmation(i);
...
void FindSetup(int idx, ENUM_TIMEFRAMES tf)
{
   ...
   if(symbols[idx].pendingOBConfirm) return;
   string sym = symbols[idx].name;
   ...
   bool shortOK = (lpHigh > 0 && close[0] < lpHigh && high[0] > lpHigh + sweepMin &&
                   ...
                   CheckCandleQuality(open[0], high[0], low[0], close[0], true));
   bool longOK  = (lpLow  > 0 && close[0] > lpLow  && low[0]  < lpLow  - sweepMin &&
                   (!checkBody || close[0] > open[0]) &&
                   CheckCandleQuality(open[0], high[0], low[0], close[0], false));
   ...
   if(InpRequireOBConfirm)
   {
      symbols[idx].pendingOBConfirm = true;
      symbols[idx].obLevel = 0;
      symbols[idx].obLastBarTime = 0;
      LogDecision(sym, "OBPending", true, "Waiting for lower-TF order-block confirmation");
      return;
   }
...
// Shared by the immediate path (InpRequireOBConfirm=false) and the OB-confirmed
// path (CheckOBConfirmation) so correlation/risk/RR checks run exactly once, at
// the moment of actual execution, against live (not signal-time) state.
void TryExecuteSetup(int idx)
{
   string sym = symbols[idx].name;
   bool correlOK = CheckCorrelationLimit(idx);
   LogDecision(sym, "Correlation", correlOK);
   if(!correlOK) { symbols[idx].setupValid = false; return; }
...
// Pending two-step entry: after a sweep+BOS confirms, wait for price to retest
// and close back through the nearest counter-swing on InpConfTF (the recent
// up-leg for a short setup, down-leg for a long) before actually executing —
// approximates "drop to a lower timeframe and wait for a fresh order block"
// instead of entering on the same bar the sweep was detected.
void CheckOBConfirmation(int idx)
{
   datetime barTime = iTime(symbols[idx].name, InpConfTF, 0);
   if(barTime == symbols[idx].obLastBarTime) return;
   symbols[idx].obLastBarTime = barTime;

   int barsSinceSweep = iBarShift(symbols[idx].name, InpConfTF, symbols[idx].sweepBarTime, false);
   if(barsSinceSweep < 0 || barsSinceSweep > InpOBConfirmTimeoutBars)
   {
      LogDecision(symbols[idx].name, "OBTimeout", false,
                  "No confirmation within " + IntegerToString(InpOBConfirmTimeoutBars) + " bars");
      symbols[idx].pendingOBConfirm = false;
      symbols[idx].setupValid = false;
      return;
   }
   ...
   int needed = InpOBLookbackBars + InpOBConfirmTimeoutBars + 2;
   int copied = CopyRates(symbols[idx].name, InpConfTF, 1, needed, m1);
   if(copied < 3) return;
   ...
   if(symbols[idx].obLevel == 0)
   {
      ...
      for(int i = 0; i < lookback; i++)
      {
         if(symbols[idx].setupDirection == DIR_SHORT)
            ...
            extreme = MathMin(extreme, m1[i].low);
      }
      symbols[idx].obLevel = extreme;
   }

   double closePrice = m1[0].close;
   bool triggered = (symbols[idx].setupDirection == DIR_SHORT) ? (closePrice < symbols[idx].obLevel)
                                                                : (closePrice > symbols[idx].obLevel);
   if(!triggered) return;

   LogDecision(symbols[idx].name, "OBConfirmed", true,
               "Closed through " + DoubleToString(symbols[idx].obLevel, 5));
   symbols[idx].pendingOBConfirm = false;
   TryExecuteSetup(idx);
}
...
bool CheckRiskAndEntry(int idx)
{
   ...
   // broker min-lot floor can force real risk well above the tiny intended target on a small account — cap it as a flat % of balance instead, so normal FX floor-rounding isn't caught, only true outliers (e.g. gold)
   if(InpMaxRiskPercentCap > 0 && symbols[idx].riskAmount > (InpMaxRiskPercentCap / 100.0) * AccountInfoDouble(ACCOUNT_BALANCE))
      return false;
...
void UpdateDashboard()
{
   ...
   CreateOrUpdateLabel("DB_DailyR", x + 8, y + line, 0, profitColor,
      "Daily R: " + DoubleToString(globalDailyR, 2) + " | $" + DoubleToString(globalDailyProfit, 2));
   line += 15;
